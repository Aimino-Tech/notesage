...
// Removed: import { API_ORIGIN } from '../remote'
import { BaseError, ApiError, NetworkError, ChatboxAIAPIError } from './errors'
// Removed: import { parseJsonOrEmpty } from '@/lib/utils'

// Define API_ORIGIN locally (adjust if needed)
const API_ORIGIN = 'https://api.chatbox.ai'; // Placeholder

// Define parseJsonOrEmpty locally
function parseJsonOrEmpty(jsonString: string): unknown {
  try {
        ...
        for (let i = 0; i < retry + 1; i++) {
            try {
                ...
                if (!res.ok) {
                    ...
                    const parsedResponse = parseJsonOrEmpty(responseText);
                    let errorCodeName: string | undefined;
                    if (typeof parsedResponse === 'object' && parsedResponse !== null && 'error' in parsedResponse && typeof parsedResponse.error === 'object' && parsedResponse.error !== null && 'code' in parsedResponse.error) {
                      errorCodeName = parsedResponse.error.code as string;
                    }
                    const chatboxAIError = ChatboxAIAPIError.fromCodeName(responseText, errorCodeName)
                    if (chatboxAIError) {
                        throw chatboxAIError
                    }
                    throw new ApiError(`Status Code ${res.status}, ${responseText}`)
                }
                return res
            } catch (e) {
