...
export default class Ollama extends Base {
    ...
    async listModels(): Promise<string[]> {
        ...
        const json = await res.json() as { models: OllamaModelInfo[] }; // Type assertion for the response structure
        if (!json || !Array.isArray(json.models)) {
             throw new ApiError(`Unexpected response structure: ${JSON.stringify(json)}`)
        }
        return json.models.map((m: OllamaModelInfo) => m.name)
    }
}
