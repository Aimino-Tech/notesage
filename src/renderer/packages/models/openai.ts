...
export default class OpenAI extends Base {
    ...
    async requestChatCompletionsStream(requestBody: Record<string, unknown>, signal?: AbortSignal, onResultChange?: onResultChange): Promise<string> {
    ...
    async requestChatCompletionsNotStream(requestBody: Record<string, unknown>, signal?: AbortSignal, onResultChange?: onResultChange): Promise<string> {
        const apiPath = this.options.apiPath || '/v1/chat/completions'
        const response = await this.post(
            `${this.options.apiHost}${apiPath}`,
            this.getHeaders(),
